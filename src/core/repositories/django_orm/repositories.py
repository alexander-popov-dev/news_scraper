...
from src.core.abstract.repositories import (
...
from src.models import Article, BrowserProfile, RawResponseData, Session, Site
...
class ArticleRepository(BaseArticleRepository):
    ...
    def get_latest_published_at(self, site_id: int) -> datetime | None:
        return (
            Article.objects.filter(site_id=site_id)
...
class SiteRepository(BaseSiteRepository):
    ...
    def get_sites_for_scraping(
        self, site_ids: list[int] | None = None
    ) -> SitesDTO | None:
        if site_ids:
...
class SessionRepository(BaseSessionRepository):
    def create_session(self, scraping_type: ScrapingDataType) -> int:
        session = Session.objects.create(
            status=SessionStatus.CREATED, scraping_type=scraping_type
        )
        return session.pk
