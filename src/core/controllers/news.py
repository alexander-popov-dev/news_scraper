...
class NewsController:
    def __init__(
        ...
        self._provider = (
            provider(profile_id=profile_id, folder_id=folder_id, close_profile=False)
            if provider
            else None
        )
        self._client = client
        ...
        with self._client(proxy=self._proxy, provider=self._provider) as client:
            ...
            for url in self._pagination:
                ...
                articles_dto = self._parser.parse_news(
                    content=response_dto.text(), page_url=url, timezone=self._timezone
                )
                ...
                for article in articles_dto.articles:
                    if (
                        ...
                        and article.published_at <= self._until_date
                    ):
                        self._pagination.stop()
                        break
